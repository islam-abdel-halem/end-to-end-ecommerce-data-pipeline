...
def process_file(filepath: str) -> tuple[list[dict[str, str | float]], set[str]]:
    ...
    processed transactions to 'processed_data.txt', and returns both the
    ...
    output_path = str(Path(filepath).with_name("processed_data.txt"))
    ...
    try:
        ...
        print(f"File operation attempt complete for: {filepath}")


def main() -> None:
    """Process the local raw transaction file and print a short summary."""
    input_path = Path(__file__).with_name("raw_transactions.txt")
    transactions, regions = process_file(str(input_path))
